...
export const handleWaterRoutine = async (
  ...
  setLoading(true);
  setMessage("Iniciando rutina de agua..."); // ✅ Corregido: dice "agua" en lugar de "comida"
  ...
  try {
    ...
    if (!pumpMessage) throw new Error("Fallo al activar la bomba"); // ✅ Corregido: dice "bomba" en lugar de "motor"
    ...
    distanciaValue = await handleReadSensor('DISTANCIA_B', setMessage, setMessageType);
    if (!distanciaValue) throw new Error("Fallo al leer sensor de distancia B");

    const finalMessage = `Rutina finalizada con éxito:\n\n` +
      `Mensaje de la bomba: ${pumpMessage}\n` + // ✅ Corregido: dice "bomba" en lugar de "motor"
      `Peso B: ${pesoValue}\n` + // ✅ Corregido: dice "Peso B" en lugar de "Peso A"
      `Distancia B: ${distanciaValue}`; // ✅ Corregido: dice "Distancia B" en lugar de "Distancia A"
