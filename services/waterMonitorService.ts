...
const WATER_WEIGHT_THRESHOLD = 7; // Umbral mínimo en gramos
...
export const checkAndDispenseWaterIfNeeded = async (): Promise<boolean> => {
  try {
    ...
    console.log(
      `💧 [WATER-MONITOR] Peso actual del agua: ${status.lastWeight}g`
    );

    // Verificar si necesita dispensar
    if (
      status.lastWeight !== null &&
      status.lastWeight < WATER_WEIGHT_THRESHOLD
    ) {
      console.log(
        `💧 [WATER-MONITOR] ¡Peso bajo detectado! (${status.lastWeight}g < ${WATER_WEIGHT_THRESHOLD}g)`
      );

      logMessage(
        "💧 Nivel de Agua Bajo",
        `El plato de agua tiene solo ${status.lastWeight}g. Se dispensará agua automáticamente.`
      );

      // Ejecutar rutina de agua
      console.log(
        "💧 [WATER-MONITOR] Iniciando dispensación automática de agua..."
      );
      ...
      logMessage(
        "✅ Agua Dispensada Automáticamente",
        "Se ha dispensado agua fresca para tu mascota."
      );
    ...
    logMessage(
      "❌ Error en Monitor de Agua",
      "Hubo un problema al verificar el nivel de agua."
    );
    return false;
  }
};
...
export const startWaterMonitor = (): void => {
  ...
  console.log(
    `💧 [WATER-MONITOR] Intervalo configurado: ${
      WATER_MONITOR_INTERVAL / 1000
    } segundos`
  );
};
...
export const setWaterMonitorEnabled = async (
  enabled: boolean
): Promise<void> => {
  const status = await loadMonitorStatus();
  ...
  console.log(
    `💧 [WATER-MONITOR] Monitor ${enabled ? "habilitado" : "deshabilitado"}`
  );
...
export const setWaterWeightThreshold = async (
  threshold: number
): Promise<void> => {
